            ...
            'rows' => array_map(static function (array $item): array {
                return [
                    'cells' => [
                        ...
                        ['html' => '<span class="fw-medium">' . htmlSC((string)$item['name']) . '</span><div class="small text-body-secondary">' . htmlSC((string)($item['remark'] ?? $item['remote_inbound_id'] ?? '')) . '</div>'],
                        ['value' => (string)($item['protocol'] ?? '-')],
                        ['value' => (string)($item['port'] ?? '-')],
                        ['html' => vpnm_status_badge(!empty($item['is_enabled']) ? 'active' : 'disabled')],
