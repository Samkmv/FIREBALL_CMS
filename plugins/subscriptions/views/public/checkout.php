        ...
        <div class="col-lg-7">
            ...
            <form class="subscriptions-checkout-panel border rounded-5 p-4 p-lg-5" action="<?= base_href('/subscriptions/payment/create') ?>" method="post">
                ...
                <div class="subscriptions-checkout-consents rounded-4 p-3 p-md-4 mb-4">
                    <label class="form-check mb-3"><input class="form-check-input" type="checkbox" name="consent_offer" value="1" required><span class="form-check-label"><?= str_replace(
                        ':offer',
                        '<a href="' . htmlSC((new \Fireball\Subscriptions\Services\PublicOfferService())->url()) . '" target="_blank" rel="noopener noreferrer">' . htmlSC(FireballPluginSubscriptions::t('subscriptions_consent_offer_link')) . '</a>',
                        htmlSC(FireballPluginSubscriptions::t('subscriptions_consent_offer_linked'))
                    ) ?></span></label>
