...
Проверка и установка доступны только пользователю с ролью `creator` и защищены CSRF. Для приватного GitHub-репозитория используется token из штатных настроек центра обновлений; token не сохраняется в manifest плагина и не выводится в журнал.
