...
function initPostEditor() {
    ...
    const labels = Object.assign({
        ...
        manageOrder: 'Управление порядком',
        orderModalTitle: 'Порядок блоков',
        orderEmpty: 'Блоков пока нет.',
        orderSave: 'Save'
    }, config.labels || {});
    ...
    const deleteModalElement = document.getElementById(app.getAttribute('data-delete-modal-id') || 'postEditorDeleteModal');
    ...
    const deleteModalConfirmButton = deleteModalElement
        ...
        : null;
    const settingsModalElement = document.getElementById(app.getAttribute('data-settings-modal-id') || 'postEditorSettingsModal');
    ...
    const settingsModalBodyElement = settingsModalElement
        ...
        : null;
    const orderModalElement = document.getElementById(app.getAttribute('data-order-modal-id') || 'postEditorOrderModal');
    const orderModalTitleElement = orderModalElement
        ? orderModalElement.querySelector('[data-editor-order-title]')
        : null;
    const orderModalListElement = orderModalElement
        ? orderModalElement.querySelector('[data-editor-order-list]')
        : null;
    const orderModalSaveButton = orderModalElement
        ? orderModalElement.querySelector('[data-editor-order-save]')
        : null;
    ...
    if (orderModalElement && bootstrapApi && bootstrapApi.Modal) {
        ui.orderModalInstance = bootstrapApi.Modal.getOrCreateInstance(orderModalElement);
    }
    ...
    if (orderModalTitleElement) {
        orderModalTitleElement.textContent = labels.orderModalTitle;
    }
    ...
    function ensureAppStructure() {
        ...
        app.innerHTML = '' +
            '<div class="fb-post-editor__bar">' +
                ...
                '</div>' +
                '<div class="fb-post-editor__bar-actions">' +
                    '<button class="btn btn-outline-secondary btn-sm rounded-pill d-inline-flex align-items-center gap-2" type="button" data-editor-open-order>' +
                        '<i class="ci-list"></i><span>' + escapeHtml(labels.manageOrder) + '</span>' +
                    '</button>' +
    ...
    function updateBlock(id, patch) {
        ...
        if (block.type === 'video') {
            block.data = normalizeVideoData(Object.assign({}, block.data, patch));
    ...
    function bindEvents() {
        ...
        app.addEventListener('click', function (event) {
            ...
            const openOrderButton = event.target.closest('[data-editor-open-order]');
        ...
        if (orderModalElement) {
            orderModalElement.addEventListener('dragstart', function (event) {
                const item = event.target.closest('[data-order-block-id]');
                if (!item) {
                    return;
                }

                ui.orderDragId = String(item.getAttribute('data-order-block-id') || '');
                item.classList.add('is-dragging');
                if (event.dataTransfer) {
                    event.dataTransfer.effectAllowed = 'move';
                    event.dataTransfer.setData('text/plain', ui.orderDragId);
                }
            });

            orderModalElement.addEventListener('dragover', function (event) {
                const item = event.target.closest('[data-order-block-id]');
                if (!item || !ui.orderDragId || String(item.getAttribute('data-order-block-id') || '') === ui.orderDragId) {
                    return;
                }

                event.preventDefault();
                const rect = item.getBoundingClientRect();
                item.classList.toggle('is-drop-before', event.clientY < rect.top + rect.height / 2);
                item.classList.toggle('is-drop-after', event.clientY >= rect.top + rect.height / 2);
            });

            orderModalElement.addEventListener('dragleave', function (event) {
                const item = event.target.closest('[data-order-block-id]');
                if (item) {
            ...
            orderModalElement.addEventListener('drop', function (event) {
                const item = event.target.closest('[data-order-block-id]');
                if (!item || !ui.orderDragId) {
                    return;
                }

                event.preventDefault();
                const targetId = String(item.getAttribute('data-order-block-id') || '');
                const rect = item.getBoundingClientRect();
                applyOrderModalMove(ui.orderDragId, targetId, event.clientY >= rect.top + rect.height / 2 ? 'after' : 'before');
            });

            orderModalElement.addEventListener('dragend', cleanupOrderModalDrag);

            if (orderModalSaveButton) {
                orderModalSaveButton.addEventListener('click', saveOrderModal);
            }
        }
    ...
    function getAllowedBlockType(type) {
        const allowedTypes = ['text', 'heading', 'image', 'video', 'audio', 'html', 'social', 'slider', 'alert', 'newsletter', 'code'];
    ...
    function openOrderModal() {
        ...
        if (ui.orderModalInstance) {
            ui.orderModalInstance.show();
            return;
        }

        orderModalElement.classList.add('is-open');
        orderModalElement.style.display = 'block';
        orderModalElement.removeAttribute('aria-hidden');
        document.body.classList.add('fb-modal-open');
    }
    ...
    function renderOrderModal() {
        if (!orderModalListElement) {
            return;
        }

        if (!state.blocks.length) {
            orderModalListElement.innerHTML = '<div class="fb-block-order-empty">' + escapeHtml(labels.orderEmpty) + '</div>';
            return;
        }

        orderModalListElement.innerHTML = state.blocks.map(function (block, index) {
            return '' +
                '<div class="fb-block-order-item" draggable="true" data-order-block-id="' + escapeAttr(block.id) + '">' +
                    '<span class="fb-block-order-item__drag" aria-hidden="true">⋮⋮</span>' +
                    '<span class="fb-block-order-item__icon">' + renderBlockIcon(block.type) + '</span>' +
                    '<span class="fb-block-order-item__body">' +
                        '<strong>' + escapeHtml(blockTitle(block.type)) + '</strong>' +
                        '<span>' + escapeHtml(orderBlockHint(block)) + '</span>' +
                        '<small>' + escapeHtml(orderBlockDescription(block, index)) + '</small>' +
                    '</span>' +
                '</div>';
        }).join('');
    }

    function orderBlockHint(block) {
        ...
        if (block.type === 'heading') {
            ...
            hint = String(data.code || '').replace(/\s+/g, ' ').trim();
        } else if (block.type === 'slider') {
            const items = normalizeSliderItems(data.items);
            hint = items[0] ? (items[0].title || items[0].text || fileNameFromPath(items[0].image || '')) : '';
        } else if (block.type === 'social') {
            const items = normalizeSocialItems(data.items);
            hint = items.map(function (item) {
                return item.label || item.network || '';
            }).filter(Boolean).join(', ');
        }

        hint = String(hint || '').replace(/\s+/g, ' ').trim();
        return truncateText(hint || blockTitle(block.type), 96);
    }

    function orderBlockDescription(block, index) {
        const pieces = ['#' + String(index + 1)];
        if (block.hidden === true) {
            pieces.push(labels.hidden);
        }

        if (block.type === 'heading') {
            pieces.push(String(block.data.level || 'h2').toUpperCase());
        } else if (block.type === 'image' && block.data.src) {
    ...
    function applyOrderModalMove(sourceId, targetId, position) {
        if (!orderModalListElement || sourceId === targetId) {
            cleanupOrderModalDrag();
            return;
        }

        const source = orderModalListElement.querySelector('[data-order-block-id="' + cssEscape(sourceId) + '"]');
        const target = orderModalListElement.querySelector('[data-order-block-id="' + cssEscape(targetId) + '"]');
        if (!source || !target) {
            cleanupOrderModalDrag();
            return;
        }
    ...
    function saveOrderModal() {
        ...
        const ids = Array.from(orderModalListElement.querySelectorAll('[data-order-block-id]')).map(function (item) {
            return String(item.getAttribute('data-order-block-id') || '');
        }).filter(Boolean);
        ...
        const sorted = ids.map(function (id) {
            return byId[id];
        }).filter(Boolean);
        ...
        state.blocks = sorted;
        ui.activeBlockId = state.blocks[0] ? state.blocks[0].id : null;
        closeOrderModal();
        render();
        sync();
    }

    function cleanupOrderModalDrag() {
        ui.orderDragId = null;
        if (!orderModalListElement) {
            return;
        }

        orderModalListElement.querySelectorAll('.is-dragging, .is-drop-before, .is-drop-after').forEach(function (item) {
