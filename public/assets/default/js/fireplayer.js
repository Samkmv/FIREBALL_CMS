(function (window, document) {
    ...
    const inferHlsPoster = function (source) {
        if (!source) {
            return '';
        }
        ...
        try {
            ...
            if (!/\/stream-[^/]+\/index\.m3u8$/i.test(url.pathname)) {
                return '';
            }
    ...
    const mergeOptions = function (element, options) {
        ...
        // Explicit poster/data-poster always wins.
        // Only infer a poster when none was provided.
        if (!merged.poster && merged.src) {
            merged.poster = inferHlsPoster(merged.src);
        }
