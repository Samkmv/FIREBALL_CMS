...
<div class="modal fade" id="fileTransferModal" tabindex="-1" aria-hidden="true" data-file-transfer-modal>
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-5 overflow-hidden">
            <form action="<?= base_href('/admin/files/action') ?>" method="post" data-fm-async-form>
                <?= get_csrf_field() ?>
                <input type="hidden" name="dir" value="<?= htmlSC($currentDir) ?>">
                <input type="hidden" name="picker" value="<?= $pickerMode ? '1' : '0' ?>">
                <input type="hidden" name="field" value="<?= htmlSC($pickerField) ?>">
                ...
                <div class="modal-header border-0 pb-0">
                    <div class="min-w-0">
                        <h2 class="modal-title fs-5" data-file-transfer-title><?= print_translation('admin_files_transfer_title_copy') ?></h2>
                        <div class="small text-body-secondary text-truncate" data-file-transfer-summary></div>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body pt-3">
                    <label class="form-label" for="fileTransferDestination"><?= print_translation('admin_files_transfer_destination_label') ?></label>
                    <select class="form-select" id="fileTransferDestination" name="destination_dir" data-file-transfer-destination>
                        <?php foreach ($destinationDirectories as $directory): ?>
                            <option value="<?= htmlSC((string)($directory['value'] ?? '')) ?>">
                                <?= htmlSC((string)($directory['label'] ?? '')) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
