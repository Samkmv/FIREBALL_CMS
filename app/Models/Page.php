...
class Page extends Model
{
    ...
    protected function ensureColumns(): void
    {
        $columns = [
            ...
            'menu_order' => "ALTER TABLE {$this->table} ADD COLUMN menu_order INT(10) UNSIGNED NOT NULL DEFAULT 0 AFTER show_in_footer",
    ...
    public function getPaginated(array $options = []): array
    {
        ...
        $items = db()->query(
            "SELECT id, title, menu_title, slug, is_published, show_in_header, show_in_footer, menu_order, created_at, updated_at
             FROM {$this->table}
             {$where}
    ...
    public function createPage(array $data): int
    {
        ...
        db()->query(
            "INSERT INTO {$this->table}
             (title, menu_title, slug, content, meta_title, meta_description, is_published, show_in_header, show_in_footer, menu_order, created_at, updated_at)
             VALUES (:title, :menu_title, :slug, :content, :meta_title, :meta_description, :is_published, :show_in_header, :show_in_footer, :menu_order, :created_at, :updated_at)",
            [
    ...
    public function getMenuPages(string $location): array
    {
        ...
        $cacheKey = $this->publicCacheKey('menu:' . $location);
        ...
        $column = $location === 'footer' ? 'show_in_footer' : 'show_in_header';
        $pages = db()->query(
        ...
        $items = array_map(static function (array $page): array {
            ...
            return [
                'id' => (int)$page['id'],
                'href' => base_href('/' . trim((string)$page['slug'], '/')),
    ...
    {
        return 'id, title, menu_title, slug, content, meta_title, meta_description, is_published, show_in_header, show_in_footer, menu_order, created_at, updated_at';
    }
