...
class FileManager
{
    ...
    public function getDirectoryData(string $relativeDir = '', array $options = []): array
    {
        ...
        return [
            ...
            'destination_directories' => $this->buildDestinationDirectories(),
    ...
    /**
     * Копирует или перемещает несколько файлов и папок в указанную директорию.
     */
    public function transferMany(array $items, string $destinationDir, string $action): int
    {
        $action = trim($action);
    ...
    /**
     * Возвращает плоский список доступных директорий для модалки переноса.
     */
    protected function buildDestinationDirectories(): array
    {
        $options = [[
            'value' => '',
            'label' => '/' . return_translation('admin_files_root'),
        ...
        usort($directories, static fn(array $left, array $right): int => strnatcasecmp((string)($left['relative_path'] ?? ''), (string)($right['relative_path'] ?? '')));

        foreach ($directories as $directory) {
            $relativePath = trim((string)($directory['relative_path'] ?? ''));
            if ($relativePath === '') {
                continue;
            }
            ...
            $options[] = [
                'value' => $relativePath,
                'label' => '/' . return_translation('admin_files_root') . '/' . $relativePath,
            ];
        }
    ...
    /**
     * Генерирует уникальное имя директории внутри указанного каталога.
     */
    protected function generateDirectoryName(string $originalName, string $absoluteDir): string
    {
        $baseName = make_slug($originalName, 'folder');
        $candidate = $baseName;
        $counter = 1;
    ...
    /**
     * Копирует директорию рекурсивно в новую локацию.
     */
    protected function copyDirectoryRecursively(string $sourceAbsoluteDir, string $targetAbsoluteDir): void
    {
        if (!is_dir($targetAbsoluteDir) && !mkdir($targetAbsoluteDir, 0755, true) && !is_dir($targetAbsoluteDir)) {
            throw new \RuntimeException(return_translation('admin_files_copy_error'));
        }

        $items = scandir($sourceAbsoluteDir);
        if ($items === false) {
            throw new \RuntimeException(return_translation('admin_files_copy_error'));
        }
    ...
    /**
     * Копирует или перемещает один объект в папку назначения.
     */
    protected function transferItem(string $relativePath, string $type, string $destinationDir, string $action): void
    {
        if ($relativePath === '' || $this->isProtectedPath($relativePath)) {
            throw new \RuntimeException(return_translation('admin_files_invalid_path'));
        }
        ...
        $absoluteDestinationDir = $this->ensureDirectoryExists($destinationDir);
        ...
        if ($type === 'directory') {
            $this->transferDirectory($relativePath, $absoluteSourcePath, $destinationDir, $absoluteDestinationDir, $action);
            return;
        }
     ...
     */
    protected function transferFile(string $relativePath, string $absoluteSourcePath, string $destinationDir, string $absoluteDestinationDir, string $action): void
    {
        ...
        $targetName = $this->generateFileName($fileName . ($extension !== '' ? '.' . $extension : ''), $extension, $absoluteDestinationDir);
        $targetAbsolutePath = $absoluteDestinationDir . '/' . $targetName;

        if ($action === 'copy') {
            if (!@copy($absoluteSourcePath, $targetAbsolutePath)) {
                throw new \RuntimeException(return_translation('admin_files_copy_error'));
            }
            return;
        }

        if (!@rename($absoluteSourcePath, $targetAbsolutePath)) {
            throw new \RuntimeException(return_translation('admin_files_move_error'));
        }
    }

    /**
     * Копирует или перемещает папку.
     */
    protected function transferDirectory(string $relativePath, string $absoluteSourcePath, string $destinationDir, string $absoluteDestinationDir, string $action): void
    {
        ...
        $directoryName = basename($relativePath);
        $targetName = $this->generateDirectoryName($directoryName, $absoluteDestinationDir);
        $targetRelativePath = ltrim($destinationDir . '/' . $targetName, '/');
        $targetAbsolutePath = $absoluteDestinationDir . '/' . $targetName;
        ...
        if ($action === 'copy') {
            $this->copyDirectoryRecursively($absoluteSourcePath, $targetAbsolutePath);
            return;
        }
