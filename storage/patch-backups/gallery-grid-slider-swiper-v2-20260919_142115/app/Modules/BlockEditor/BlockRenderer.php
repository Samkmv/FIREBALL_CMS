...
final class BlockRenderer
{
    ...
    private function gallery(array $data, string $type): string
    {
        ...
        // FIREBALL_GALLERY_SWIPER_V1
        //
        // В редакторе gallery остаётся удобной сеткой для выбора изображений,
        // а на публичной странице рендерится как Cartzilla/Swiper-карусель.
        //
        // Runtime-классы Swiper (swiper-initialized, swiper-slide-active,
        // transform, width и swiper-notification) намеренно не выводятся:
        // их добавляет сам Swiper после инициализации.
        if ($type === 'gallery') {
            $slides = [];
            ...
            $baseId = 'fb-gallery-' . $gallerySequence;
            $swiperId = $baseId . '-swiper';
            $wrapperId = $baseId . '-wrapper';
            $prevId = $baseId . '-prev';
            $nextId = $baseId . '-next';
            $paginationId = $baseId . '-pagination';

            $multiple = count($slides) > 1;
            $showArrows = !array_key_exists('showArrows', $data) || !empty($data['showArrows']);
            $showBullets = !array_key_exists('showBullets', $data) || !empty($data['showBullets']);
            ...
            $swiperJson = json_encode(
                $swiperConfig,
                JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE
            );
            if (!is_string($swiperJson) || $swiperJson === '') {
                $swiperJson = '{}';
            }

            $html = '<div class="position-relative py-3 py-md-4 px-5 mb-3" data-fb-slider-shell="1">';

            if ($multiple && $showArrows) {
                $html .=
                    '<button type="button" id="' . htmlSC($prevId) . '"' .
                    ' class="btn btn-icon btn-outline-secondary rounded-circle animate-slide-start position-absolute top-50 start-0 translate-middle-y mt-n3 z-2"' .
                    ' data-fb-slider-prev="1"' .
                    ' aria-label="Previous slide"' .
                    ' aria-controls="' . htmlSC($wrapperId) . '">' .
                    '<i class="ci-chevron-left fs-lg animate-target" aria-hidden="true"></i>' .
                    '</button>';

                $html .=
                    '<button type="button" id="' . htmlSC($nextId) . '"' .
                    ' class="btn btn-icon btn-outline-secondary rounded-circle animate-slide-end position-absolute top-50 end-0 translate-middle-y mt-n3 z-2"' .
                    ' data-fb-slider-next="1"' .
                    ' aria-label="Next slide"' .
                    ' aria-controls="' . htmlSC($wrapperId) . '">' .
                    '<i class="ci-chevron-right fs-lg animate-target" aria-hidden="true"></i>' .
                    '</button>';
            }
            ...
            $html .=
                ...
                ' id="' . htmlSC($swiperId) . '"' .
                ' data-fb-slider="1"' .
                ' data-swiper="' . htmlSC($swiperJson) . '">' .
                '<div class="swiper-wrapper" id="' . htmlSC($wrapperId) . '">';

            foreach ($slides as $slide) {
                $html .=
                    '<div class="swiper-slide">' .
                    '<div class="ratio" style="--cz-aspect-ratio: calc(505 / 728 * 100%)">' .
                    '<img src="' . htmlSC($slide['src']) . '"' .
                    ' class="rounded-4"' .
                    ' alt="' . htmlSC($slide['alt']) . '"' .
                    ' loading="lazy">' .
                    '</div>';
            ...
            if ($multiple && $showBullets) {
                $html .=
                    '<div id="' . htmlSC($paginationId) . '"' .
                    ' class="swiper-pagination position-static mt-3"></div>';
            }
        ...
        // Старый block type "slider" пока оставляем без изменения поведения.
